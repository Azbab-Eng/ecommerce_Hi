    ...
    const listProductDetails = async(id) =>{
      console.log(id)
        try {
          ...
          setError(error.response.data.message || error.message || "Could not get the Product" )
        }
    }
    ...
    const UpdateProduct =async ()=>{
        ...
        const config = {
          ...
          setLodingUpdate(true)                           
            ...
            const {data} = await axios.put(`${PORT}/products/${product._id}`,formData, config) 
            setLodingUpdate(true) 
            setProduct(data)
            setSuccessUpdate(data.message || "Product updated")
            setMessage(data.message || "Product updated")
        } catch (error) {
          setErrorUpdate(error.response.data.message || error.message || "Could not get the Product" )           
          setMessage(error.response.data.message || error.message || "Could not get the Product" )           
        }
    }
